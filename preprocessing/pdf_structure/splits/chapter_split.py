...
class ChapterSplit(LegalUnitSplit):
    ...
    def __init__(self, split: TextSplit, is_hidden: bool = False):
        ...
        self.articles: List[ArticleSplit] = []
    ...
    def title(self):
        ...
        title_search = re.search(
            r'(R\s*o\s*z\s*d\s*z\s*i\s*a\s*[łl]\s+[1-9a-zA-Z]+)\s*\s*((?:.+\n)*?)^\s*(?=R\s*o\s*z\s*d\s*z\s*i\s*a\s*[łl]|Art\.)',
            self.split.text,
            flags=re.MULTILINE)

        if title_search is None:
            match_of_identification = re.match(self._can_erase_number_pattern(), self.split.text)
            outside_index_start = self.split.start_index + match_of_identification.start(0)
            outside_index_end = self.split.start_index + match_of_identification.end(0) - 1
            return self.split.build_text_split_from_indexes(outside_index_end + 1, self.split.end_index).text
    ...
    def _can_erase_number_pattern(cls):
        return r'^R\s*o\s*z\s*d\s*z\s*i\s*a\s*[łl]\s+([1-9a-zA-Z]+)'

    def split_item_for_further_processing(self):
        if self.is_hidden:
            return self.split

        outside_search = re.search(
            r'(R\s*o\s*z\s*d\s*z\s*i\s*a\s*[łl]\s+[1-9a-zA-Z]+)\s*\s*((?:.+\n)*?)^\s*(?=R\s*o\s*z\s*d\s*z\s*i\s*a\s*[łl]|Art\.)',
            self.split.text,
            flags=re.MULTILINE)
