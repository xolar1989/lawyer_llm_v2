...
class SectionSplitter(AbstractDocumentSplitter[SectionSplit]):
    ...
    def split_function(self, text):
        return re.finditer(
                r'(?<![-–])(?<!\sart\.\d\d\d\s)(?<!\sstr\.\s)(?<!\sstr\.\s\s)(?<!\sstr\.\s\s\s)(?<!\salbo\s)(?<!\salbo\s\s)(?<!\salbo\s\s\s)(?<!\slub\s)(?<!\slub\s\s)(?<!\si\s)(?<!\si\s\s)(?<!\si\s\s\s)(?<!\spkt\s)(?<!\spkt\s\s)(?<!\spkt\s\s\s)(?<!\sust\.\s)(?<!\sust\.\s\s)(?<!\sust\.\s\s\s)(?<!\sart\.\s)(?<!\sart\.\s\s)(?<!\sart\.\s\s\s)(?<!\spoz\.\s)(?<!\spoz\.\s\s)(?<!\spoz\.\s\s\s)^((?:[\<§\[]*\s*)?\d{1,2}[a-zA-ZĄĆĘŁŃÓŚŹŻąćęłńóśźż⁰¹²³⁴⁵⁶⁷⁸⁹]{0,4}\.\s*(?=[A-ZĄĆĘŁŃÓŚŹŻ⁰¹²³⁴⁵⁶⁷⁸⁹⁾\(\[\<])[\s\S]*?)(?=(?<![-–])(?<!\sart\.\s[\da-ząćęłńóśźż][\da-ząćęłńóśźż][\da-ząćęłńóśźż]\s)(?<!\sart\.\s[\da-ząćęłńóśźż][\da-ząćęłńóśźż][\da-ząćęłńóśźż]\s\s)(?<!\sart\.\s[\da-ząćęłńóśźż][\da-ząćęłńóśźż][\da-ząćęłńóśźż][\da-ząćęłńóśźż]\s\s)(?<!\sart\.\s[\da-ząćęłńóśźż][\da-ząćęłńóśźż]\s)(?<!\sart\.\s[\da-ząćęłńóśźż][\da-ząćęłńóśźż]\s\s)(?<!\sart\.\s[\da-ząćęłńóśźż]\s)(?<!\sart\.\s[\da-ząćęłńóśźż]\s\s)(?<!\salbo\s)(?<!\salbo\s\s)(?<!\salbo\s\s\s)(?<!\sstr\.\s)(?<!\sstr\.\s\s)(?<!\sstr\.\s\s\s)(?<!\slub\s)(?<!\slub\s\s)(?<!\si\s)(?<!\si\s\s)(?<!\si\s\s\s)(?<!\spkt\s)(?<!\spkt\s\s)(?<!\spkt\s\s\s)(?<!\sust\.\s)(?<!\sust\.\s\s)(?<!\sust\.\s\s\s)(?<!\sart\.\s)(?<!\sart\.\s\s)(?<!\sart\.\s\s\s)(?<!\spoz\.\s)(?<!\spoz\.\s\s)(?<!\spoz\.\s\s\s)^((?:[\<§\[]*\s*)?\d{1,2}[a-zA-ZĄĆĘŁŃÓŚŹŻąćęłńóśźż⁰¹²³⁴⁵⁶⁷⁸⁹]{0,4}\.\s*(?=[A-ZĄĆĘŁŃÓŚŹŻ⁰¹²³⁴⁵⁶⁷⁸⁹⁾\(\[\<]))|\Z)',
                text,
